...
public:
	...
	BOOL Move(INT id, WindowDraw & wd)
	{
		...
		if(y){
			...
			//проходим по каждому игроку и проверяем, если уно, нажал ли он на кнопку
			for (size_t i = 0; i < pl.size(); i++)
			{
				if (pl[i]->ForgotUno()){
					GiveOutCard(pl[cureentTurn]);
	...
	BOOL IsPlayerHasNeededCard(Player * pl, BOOL sCard)
		//есть ли у игрока нужная карта для ответа
	{
		for (size_t i = 0; i < pl->size; i++)
		{
	...
	BOOL IsCorrectNames(vector<HWND> &plnames) {//проверка ввода имен игроков
		...
		for (size_t i = 0; i < pl.size() && i < plnames.size(); i++)
		{
			...
			if (l < 2){
				delete[] tmp;
				MessageBox(NULL, TEXT("Длина имени должна быть от 2 до 11 символов"),TEXT("Ошибка!"), MB_OK);
				return FALSE;
			}
